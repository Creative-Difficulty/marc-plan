<script lang="ts">
	...
	$: formInputsHaveError = form?.error !== undefined && form?.error !== null;
...
<div class="flex justify-center items-center h-screen w-screen bg-gradient-to-r from-red-500 to-orange-500">
		...
		>
			<Card.Content class="space-y-2">
				<div class="space-y-1">
					...
					<Input type="text" id="user_email" name="user_email" class={progressiveWrongEmail || formInputsHaveError ? 'border-2 border-rose-600' : ''} />
					{#if progressiveWrongEmail}
						<p class="text-sm text-red-600">E-Mail ist nicht gültig</p>
					{:else if formInputsHaveError}
						<p class="text-sm text-red-600">E-Mail und/oder Passwort sind falsch</p>
					{/if}
				</div>
				<div class="space-y-1">
					<Label for="user_password">Password</Label>
					<Input id="user_password" name="user_password" type="password" class={formInputsHaveError || passwordTooShort ? 'border-2 border-rose-600' : ''} />
					{#if passwordTooShort}
						<p class="text-sm text-red-600">Passwort ist zu kurz</p>
					{:else if formInputsHaveError}
						<p class="text-sm text-red-600">Passwort und/oder E-Mail sind falsch</p>
					{/if}
				</div>
			</Card.Content>
			<Card.Footer>
